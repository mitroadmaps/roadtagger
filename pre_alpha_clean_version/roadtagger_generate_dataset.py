...
def generate_dataset(config):
        ...
        if os.path.isfile(output_folder+"/sat_16384.png") == False:
                ...
                img2 = cv2.resize(src=img, dsize=(img_size, img_size))
        ...
        for nodeid, node in OSMMap.nodedict.items():
...
def graphDensify(input_pickle_graph, vis=True):
        ...
        map1 = pickle.load(open(input_pickle_graph, "rb"))
        ...
        if vis == True:
                ...
                for k,_ in node_neighbors.items():
...
def generate_per_node_image(config, input_sat = "sat_16384.png", output_name = "tiles", scale = 1.0):
        ...
        image = matplotlib.pyplot.imread(output_folder+"/"+input_sat).astype(np.uint8)
        try:
                annotation = pickle.load(open(output_folder+"/annotation.p", "rb"))
        except:
                annotation = pickle.load(open(output_folder+"/annotation_osm.p", "rb"))
        ...
        for k,v in annotation.items():
                ...
                subimage = cv2.resize(src = subimage, dsize=(272*2, 272*2)).astype(np.uint8)
                ...
                img = scipy.ndimage.rotate(subimage, angle)

                center = np.shape(img)[0]/2
                print(center)

                r = 192 # 384
                result = img[int(center-r):int(center+r), int(center-r):int(center+r),:]
...
def annotate_dataset_osm(config):
        ...
        if os.path.isfile(annotation_file):
                ...
                raw_img = matplotlib.pyplot.imread(output_folder+"/sat_16384.png").astype(np.uint8)
                overview = cv2.resize(src=raw_img, dsize=(4096, 4096)).astype(np.uint8)


                for loc, node in roadNetwork.nodes.items():
                        # draw a circle
                ...
                pickle.dump(annotation, open(output_folder+"/annotation_osm.p", "wb"))
...
class annotate_dataset():
        def __init__(self, config, osm_auto = False):
                ...
                self.raw_img = matplotlib.pyplot.imread(output_folder+"/sat_16384.png").astype(np.uint8)
                self.overview = cv2.resize(src=self.raw_img, dsize=(4096, 4096)).astype(np.uint8)
                self.region = region 
