...
class RoadNetwork():
        ...
        def GetImages(self):
                if self.images is None:
                        ...
                        img = imread(self.image_file).astype(np.float32)/255.0
                        img_road = imread(self.road_image_file).astype(np.float32)/255.0
        ...
        def RandomSample(self, size = 512, name = "default"):
                ...
                # load images 
                gps_img = imread(self.image_file)
                road_img = imread(self.road_image_file)
        ...
        def loadAnnotation(self, filename, osm_auto = False, preload_img = False, lane_numbers = [0]*6, root_folder="", force_osm = False ):
                ...
                if force_osm:
                        ...
                        self.annotation = pickle.load(open(root_folder+"/"+output_folder+"/annotation_osm.p", "rb"))
                else:

                        for a_filename in  ["/annotation_fixed.p", "/annotation.p", "/annotation_osm.p"]:

                                try:
                                        self.annotation = pickle.load(open(root_folder+"/"+output_folder+a_filename, "rb"))
                                        print(a_filename)
                                        break
                                except:
                ...
                if preload_img == True:
                        ...
                        for k in self.annotation.keys():
                                img = imread(output_folder + "/tiles/img_%d.png" % k).astype(np.float32)/255.0 
...
def GenerateMicroBenchmark():
        ...
        for k, va in roadnetwork.nodes.items():
                v = {}
...
def SubRoadNetworkAddImageForMicrobenchmark(subnet, img):
        ...
        for k,v in subnet.parentRoadNetowrk.nodes.items():
                nid = v[0]
                loc_image = v[1]
...
class SubRoadNetwork():
        def __init__(self, parentRoadNetowrk, train_cnn_only = False, train_cnn_batch = 64,train_cnn_preload = 256, graph_size = 256, augmentation = True, search_mode = 0, partial = False, noLeftRight = False, remove_adjacent_matrix = 0, output_folder = None, lane_balance_factor=None, reseed = False, must_have_lane = False, must_have_lane_change = False, no_image = False, tiles_name = "tiles" ):
                ...
                if no_image == False:
                        ...
                        if has_titles == False:
                                large_image = imread(output_folder+"/sat_16384.png").astype(np.uint8)
                        ...
                        for nid in self.subGraphNoadList[:self.nonIntersectionNodeNum]:
                                if has_titles:
                                        if self.parentRoadNetowrk.preload_img is not None:
                                                ...
                                                try:
                                                        img = imread(output_folder + "/"+tiles_name+"/img_%d.png" % nid).astype(np.float32)/255.0 
                                                except:

                                                        img = imread("/data/songtao/DeepRoadMateinfo/"+ self.config["folder"] + "/"+tiles_name+"/img_%d.png" % nid).astype(np.float32)/255.0 
        ...
        def UpdateConsistency(self, output_lane_number, output_road_type, decay = 0.99):
                ...
                for i in range(self.nonIntersectionNodeNum):
        ...
        def SegmentSmoothSlidingWindow(self, outputs, window_size = 3):
                for seg_id, seg in self.parentRoadNetowrk.segments.items():
                        if len(seg)<=2+window_size-1:
                                continue

                        new_labels_lane = [[0] * 6 for _ in range(len(seg))]
                        new_labels_roadtype = [[0] * 2 for _ in range(len(seg))]

                        for start_pos in range(1,len(seg)-1):
        ...
        def SegmentAccuracy(self, outputs):
                ...
                for i in range(len(segments_score_lane)):
                ...
                for i in range(len(segments_score_roadtype)):
