...
class SubRoadNetwork():
	...
	def Dump2Json(self, outputs, output_file = "result.json"):
		result = []
		inds = {}
		for edge in self.parentRoadNetowrk.edges:
			if edge[0] in self.subGraphNoadList and edge[1] in self.subGraphNoadList:
				...
				if ind0 < self.nonIntersectionNodeNum and ind0 not in inds:
					num_of_lane = np.argmax(outputs[1][ind0,:].reshape((6))) + 1
					roadtype = 0 if outputs[6][ind0,0] > 0.5 else 1 
					result.append([loc0, int(num_of_lane), roadtype,  [float(outputs[1][ind0,j]) for j in range(6) ], [float(outputs[6][ind0,0]), float(outputs[6][ind0,1]) ], ind0])
					inds[ind0] = 1

				if ind1 < self.nonIntersectionNodeNum and ind1 not in inds:
					num_of_lane = np.argmax(outputs[1][ind1,:].reshape((6))) + 1
					roadtype = 0 if outputs[6][ind1,0] > 0.5 else 1
					result.append([loc1, int(num_of_lane), roadtype, [float(outputs[1][ind1,j]) for j in range(6) ],  [float(outputs[6][ind1,0]), float(outputs[6][ind1,1]) ], ind1])
					inds[ind1] = 1
