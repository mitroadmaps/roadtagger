...
def Render(GPSMap, CNNMap, OSMMap, region, filename = "RoadExploreCombinerAllstonTOPOOutput.png", id1 = 0, id2 = 0):
	font = cv.InitFont(cv.CV_FONT_HERSHEY_SIMPLEX, 0.4, 0.4, 0, 1, 4)
	...
	overlay_dat = scipy.ndimage.imread("allston_CNNInputSatellite.png")
	
	
	sizex = np.shape(overlay_dat)[0]
	sizey = np.shape(overlay_dat)[1]

	map_cv = cv.fromarray(overlay_dat)
	...
	for edgeId, edge in GPSMap.edges.iteritems():
		...
		# tid = GPSMap.nodeHashReverse[n1] / 10000000
		# nid = GPSMap.nodeHashReverse[n1] % 10000000

		# edgeScore = GPSMap.forest[tid][nid]['edgeScore']
		...
		# if GPSMap.edgeInt[edgeId] is not None:

		# 	for i in range(1,len(GPSMap.edgeInt[edgeId])):
		# 		ilatA, ilonA = Coord2Pixels(latold, lonold, min_lat, min_lon, max_lat, max_lon, sizex, sizey)
		# 		ilatB, ilonB = Coord2Pixels(GPSMap.edgeInt[edgeId][i][0], GPSMap.edgeInt[edgeId][i][1], min_lat, min_lon, max_lat, max_lon, sizex, sizey)
		# 		cv.Line(map_cv, (ilonA, ilatA), (ilonB, ilatB), color, 1, cv.CV_AA)


		# 		latold = GPSMap.edgeInt[edgeId][i][0]
		# 		lonold = GPSMap.edgeInt[edgeId][i][1]



		# 	cv.Line(map_cv, (ilonB, ilatB), (ilon, ilat), color, 1, cv.CV_AA)
		# else:
		cv.Line(map_cv, (ilon2, ilat2), (ilon, ilat), color, 1, cv.CV_AA)
		...
		#cv.PutText(map_cv, str(n2), (ilon-5,ilat+15), font, (255,255,0)) 

		#cv.Line(map_cv, (ilon-dlon+nlon, ilat-dlat+nlat), (ilon, ilat), color, 2, cv.CV_AA)
		#cv.Line(map_cv, (ilon-dlon-nlon, ilat-dlat-nlat), (ilon, ilat), color, 2, cv.CV_AA)
	...
	# 	cv.Circle(map_cv, (ilon,ilat), 2,(255,0,0),thickness=2)
	# 	cv.Circle(map_cv, (ilon2,ilat2), 2,(255,0,0),thickness=2)



	######## OSM

	for edgeId, edge in OSMMap.edges.iteritems():
		...
		lat1 = OSMMap.nodes[n1][0]
		lon1 = OSMMap.nodes[n1][1]

		lat2 = OSMMap.nodes[n2][0]
		lon2 = OSMMap.nodes[n2][1]
	...
	# marbles = GPSMap.TOPOWalk(42427, step = 0.00010, r=0.00300, direction = True)

	# print(marbles)

	# for pos in marbles:
	# 	ilat, ilon = Coord2Pixels(pos[0], pos[1], min_lat, min_lon, max_lat, max_lon, sizex, sizey)
	# 	cv.Circle(map_cv, (ilon,ilat), 1,(0,0,255),thickness=2)


	# id1 = 25430
	# id2 = 414

	marbles = GPSMap.TOPOWalk(id1, step = 0.00005, r = 0.00300,direction = True)
	holes = OSMMap.TOPOWalk(id2, step = 0.00005, r = 0.00300,direction = True)

	# for pos in holes:
	# 	ilat, ilon = Coord2Pixels(pos[0], pos[1], min_lat, min_lon, max_lat, max_lon, sizex, sizey)
	# 	cv.Circle(map_cv, (ilon,ilat), 3,(0,0,255),thickness=1)


	matchedNum = 0

	for marble in marbles:
		ilat, ilon = Coord2Pixels(marble[0], marble[1], min_lat, min_lon, max_lat, max_lon, sizex, sizey)
		cv.Circle(map_cv, (ilon,ilat), 2,(255,0,0),thickness=2)

		for hole in holes:
			if distance(marble, hole) < 0.00010:
				matchedNum += 1
				ilat, ilon = Coord2Pixels(marble[0], marble[1], min_lat, min_lon, max_lat, max_lon, sizex, sizey)
				cv.Circle(map_cv, (ilon,ilat), 2,(0,255,0),thickness=2)
				break


	for hole in holes:
		ilat, ilon = Coord2Pixels(hole[0], hole[1], min_lat, min_lon, max_lat, max_lon, sizex, sizey)
		cv.Circle(map_cv, (ilon,ilat), 5,(255,0,0),thickness=2)

		for marble in marbles:
			if distance(marble, hole) < 0.00010:
				matchedNum += 1
				ilat, ilon = Coord2Pixels(hole[0], hole[1], min_lat, min_lon, max_lat, max_lon, sizex, sizey)
				cv.Circle(map_cv, (ilon,ilat), 5,(0,0,255),thickness=2)
				break

	ilat, ilon = Coord2Pixels(GPSMap.nodes[id1][0], GPSMap.nodes[id1][1], min_lat, min_lon, max_lat, max_lon, sizex, sizey)
	cv.Circle(map_cv, (ilon,ilat), 6,(255,255,255),thickness=3)
	...
	# 	islat, islon = Coord2Pixels(nodes[start_id][0],nodes[start_id][1],min_lat, min_lon, max_lat, max_lon, sizex, sizey)
	# 	ielat, ielon = Coord2Pixels(nodes[end_id][0],nodes[end_id][1],min_lat, min_lon, max_lat, max_lon, sizex, sizey)

	# 	cv.Line(map_cv, (islon, islat), (ielon, ielat), (0,255,0), 2, cv.CV_AA)


	img=np.asarray(map_cv).astype(np.int)
...
def TOPOGenerateStartingPoints(OSMMap, check = True, density = 0.00050, region = None, image = None, direction = False, metaData = None, RoadNetworkCallback= None, margin = 1.0 ):
	result = []
				...
				for j in range(len(node_list)-1):
					...
					if alpha * dist >= dists[j] and alpha * dist <= dists[j+1]:
						...
						if img != None:
							...
							lat_mergin = 0.07*(region[2]-region[0]) * margin
							lon_mergin = 0.07*(region[3]-region[1]) * margin
							...
							lat_mergin = 70*0.5/111111.0 * margin
							lon_mergin = 70*0.5/111111.0 / math.cos(math.radians(region[0])) * margin

							if lat-region[0] > lat_mergin and region[2] - lat > lat_mergin and lon-region[1] > lon_mergin and region[3] - lon > lon_mergin:
								road_segments.append((int(lat*111111),int(lon*111111), road_info))



			if RoadNetworkCallback is not None:
				for j in range(len(road_segments)-1):
					rn1 = RoadNetworkCallback.AddNode(road_segments[j][0], road_segments[j][1], road_segments[j][2])
					rn2 = RoadNetworkCallback.AddNode(road_segments[j+1][0], road_segments[j+1][1], road_segments[j+1][2])
					RoadNetworkCallback.AddEdge(rn1, rn2)
...
# 'MapNodeID', 25574, 'OSMNodeID', 9026, 'Precesion', 0.9313725490196079, 'Recall', 0.8472222222222222, 'Avg Precesion', 0.9757362243153039, 'Avg Recall', 0.7184954049243392)
...
if __name__ == "__main__":
	...
	region = data[0]
	roadMapGPS = data[1]
	roadMapCNN = pickle.load(open(sys.argv[2], "rb"))

	IGCCNN = (pickle.load(open(sys.argv[3], "rb")))[1]

	print("Load Pickle Ready!")


	# make the CNN part bi-direction 

	# for nodename, nodeid in IGCCNN.nodeHasH.iteritems():
	# 	if str(nodename).startswith('Tmp') or str(nodename).startswith('CNN'):
	...
	print("Load OSM Ready!")


	
	CNNRoadGraph = splfy.RoadGraph()
	...
	for edge in edges.values():
		...
		CNNRoadGraph.addEdge(n1, nodes[n1][0], nodes[n1][1], n2, nodes[n2][0], nodes[n2][1])



	CNNRoadGraph.ReverseDirectionLink()

	for node in CNNRoadGraph.nodes.keys():
		CNNRoadGraph.nodeScore[node] = 100

	for edge in CNNRoadGraph.edges.keys():
		CNNRoadGraph.edgeScore[edge] = 100

	print("Load CNN Result Ready!")

	#TOPO(roadMapGPS, OSMRoadGraph)




	#Render(roadMapGPS, roadMapCNN, OSMRoadGraph, region)
	#exit()



	CNNList = TOPOGenerateList(CNNRoadGraph, OSMRoadGraph, check=False, region=region, image=sys.argv[4], direction = False)
	GPSList = TOPOGenerateList(roadMapGPS, OSMRoadGraph, region=region, image=sys.argv[4], direction = False)
	IGCCNNList = TOPOGenerateList(IGCCNN, OSMRoadGraph, region=region, image=sys.argv[4], direction = False)

	print(len(CNNList))
	print(len(GPSList))
	print(len(IGCCNNList))

	newCNNList = {}
	newGPSList = {}
	newIGCCNNList = {}

	for k,v in CNNList.iteritems():
		if k in GPSList.keys() and k in IGCCNNList.keys():
			newCNNList[k] = v

	for k,v in GPSList.iteritems():
		if k in CNNList.keys() and k in IGCCNNList.keys():
			newGPSList[k] = v

	for k,v in IGCCNNList.iteritems():
		if k in CNNList.keys() and k in GPSList.keys():
			newIGCCNNList[k] = v



	print(len(newCNNList))
	print(len(newGPSList))
	print(len(newIGCCNNList))


	# print(newIGCCNNList)
	# print(newIGCCNNList[12130])
	# print(newIGCCNNList[11650])
	# print(newIGCCNNList[7529])
	# print(newIGCCNNList[6520])

	#Render(IGCCNN, roadMapCNN, OSMRoadGraph, region, filename = "AllstonOSM.png", id1= 39082, id2= 12130) # 39082, 'OSMNodeID', 12130, 'Precesion', 0.39655172413793105, 'Recall', 1.0, '
	#exit()
	# Render(IGCCNN, roadMapCNN, OSMRoadGraph, region, filename = "TOPOexample2.png", id1= 12984, id2= 11650) # 12984, 'OSMNodeID', 11650, 'Precesion', 0.13142857142857142, 'Recall', 0.06518282988871224,
	# Render(IGCCNN, roadMapCNN, OSMRoadGraph, region, filename = "TOPOexample3.png", id1= 77415, id2= 7529) # 77415, 'OSMNodeID', 7529, 'Precesion', 0.9795275590551181, 'Recall', 0.4655380894800484, 'Avg Preces
	# Render(IGCCNN, roadMapCNN, OSMRoadGraph, region, filename = "TOPOexample4.png", id1= 26671, id2= 6520) # 26671, 'OSMNodeID', 6520, 'Precesion', 0.9949409780775716, 'Recall', 0.5654761904761905, 'Avg
	# #Render(roadMapGPS, roadMapCNN, OSMRoadGraph, region, filename = "TOPOexample5.png", id1= 12895, id2= 7670)


	# 32969, 'OSMNodeID', 8558, 
	# 48192, 'OSMNodeID', 6130,
	# 68101, 'OSMNodeID', 11059, 'Precesion', 0.4444444444444444, 'Recall', 0.10199
	#  25425, 'OSMNodeID', 13161, 'Precesion', 0.5578231292517006, 'Recall', 
	# ', 68090, 'OSMNodeID', 3544, 'Precesion', 0.746268656716418, 'Recall', 0.3439024390243902


	# Render(roadMapGPS, roadMapCNN, OSMRoadGraph, region, filename = "TOPOexample20.png", id1= 32969, id2= 8558)
	# Render(roadMapGPS, roadMapCNN, OSMRoadGraph, region, filename = "TOPOexample21.png", id1= 68101, id2= 11059)
	# Render(roadMapGPS, roadMapCNN, OSMRoadGraph, region, filename = "TOPOexample22.png", id1= 25425, id2= 13161)
	# Render(roadMapGPS, roadMapCNN, OSMRoadGraph, region, filename = "TOPOexample23.png", id1= 68090, id2= 3544)
	# Render(roadMapGPS, roadMapCNN, OSMRoadGraph, region, filename = "TOPOexample24.png", id1= 26694, id2= 2007)

	# exit()


	# print("IGCCNN 0.00010")
	# TOPOWithList(IGCCNN, OSMRoadGraph, newIGCCNNList, threshold = 0.00010)

	# print("GPS 0.00015")
	# TOPOWithList(roadMapGPS, OSMRoadGraph, newGPSList, threshold = 0.00015)


	# exit()

	print("IGCCNN 0.00015")
	TOPOWithList(IGCCNN, OSMRoadGraph, newIGCCNNList, threshold = 0.00015)

	# print("IGCCNN 0.00020")
	# TOPOWithList(IGCCNN, OSMRoadGraph, newIGCCNNList, threshold = 0.00020)
	...
	print("CNN 0.00010")
	TOPOWithList(CNNRoadGraph, OSMRoadGraph, newCNNList, threshold = 0.00010)

	print("GPS 0.00010")
	TOPOWithList(roadMapGPS, OSMRoadGraph, newGPSList, threshold = 0.00010)


	print("CNN 0.00015")
	TOPOWithList(CNNRoadGraph, OSMRoadGraph, newCNNList, threshold = 0.00015)
	...
	print("CNN 0.00020")
	TOPOWithList(CNNRoadGraph, OSMRoadGraph, newCNNList, threshold = 0.00020)

	print("GPS 0.00020")
	TOPOWithList(roadMapGPS, OSMRoadGraph, newGPSList, threshold = 0.00020)
	

	#TOPO(CNNRoadGraph, OSMRoadGraph)


	#roadMapGPS.BiDirection()

	# Necessary
	#roadMapGPS.ReverseDirectionLink()



	#print(roadMapGPS.nodeLink[21341])
	#print(roadMapGPS.nodeLink[299])

	# for edge in roadMapGPS.edges.values():
	# 	n1 = edge[0]
	# 	n2 = edge[1]
	# 	score = roadMapGPS.edgeScore[roadMapGPS.edgeHash[n1*10000000 + n2]]
	# 	print(n1, score, roadMapGPS.nodeScore[n1], "          ", n2, score, roadMapGPS.nodeScore[n2])
		




	# roadMapGPS.CombineSimilarSnippets(region=[42.3601, -71.1208, 42.3629, -71.1169])
	# roadMapGPS.CombineSimilarSnippets(region=[42.3464, -71.0957, 42.3555, -71.0852])

	# roadMapGPS.CombineSimilarSnippets()

	#marbles = roadMapGPS.TOPOWalk(47431)
	...
	# pickle.dump([data[0],roadMapGPS], open("RoadExploreGPSRoadMap6000SFull", "wb"))
